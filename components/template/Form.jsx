...
function Form({ state, setState, handler, loading }) {
  ...
  return (
    <div className={styles.form}>
      <p className={styles.title}>{router.asPath == '/auth/signup' ? 'ایجاد حساب کاربری' : 'ورود به حساب کاربریی'}</p>
      ...
      <div>
        <input type="text" placeholder="شماره تلفن" value={state.phone} name="phone" onChange={(e) => changeHandler(e.target.name, e.target.value)} />
      </div>
      ...
      <div>
        ...
        />
        <button onClick={() => setShowPass((prev) => !prev)}>{showPass ? <FaRegEyeSlash /> : <FaRegEye />}</button>
      </div>
      ...
      {router.asPath == '/auth/signup' && (
        <div>
          <input
            ...
            onChange={(e) => changeHandler(e.target.name, e.target.value)}
          />
          <button onClick={() => setShowPass2((prev) => !prev)}>{showPass2 ? <FaRegEyeSlash /> : <FaRegEye />}</button>
        </div>
      )}
